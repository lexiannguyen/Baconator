...
class GraphNode():
    ...
    def connect(self, dest_node, weight): #HW
        """Connects this node to another node.

        undirected graph - connects other way too
        """
        ...
        if dest_node not in self.edges:
            ...
            self.edges[dest_node].append(weight)
            dest_node.back_edges[self].append(weight)
    ...
    def all_edges(self):
        ...
        for dest_node, weights in self.edges.items():
            for weight in weights:
                yield dest_node, weight
...
class Graph():
    ...
    def connect(self, source_name, dest_name, weight=1):
        ...
        if source_name not in self:
            self[source_name] = GraphNode(source_name)
        if dest_name not in self:
            self[dest_name] = GraphNode(dest_name)
        source = self[source_name]
        dest = self[dest_name]
        source.connect(dest, weight) #dest is a node, weight is the movie
    ...
    def newbfs_traversal(self, node_name):
        # edited to include weight 
        # edit previous ? 
        """Does an iterative breadth first search traversal"""
        for n in self.nodes:
            self.nodes[n].color = "white"
            self.nodes[n].previous = None #making it node and weight
        start_node = self[node_name]
        queue = deque()
        start_node.color = "gray"
        queue.appendleft(((start_node), None)) #initial weight for start node
        while len(queue) != 0:
            node, weight = queue.pop()
            for (dest, dest_weight) in node.all_edges():
                if dest.color == "white":
                    dest.color = "gray"
                    dest.previous = node 
                    queue.appendleft((dest, dest_weight))
            node.color = "black"
            yield node, weight
